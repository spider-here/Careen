...
class HomeState extends State {
  ...
  _confirmPickup() async {
    ...
    if(_nearestRide!=null){
        ...
        showDialog(
            ...
            builder: (BuildContext context) {
              return Dialog(
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(20.0)), //this right here
                  child: _customComponents.cCard(Stack(
                    children: [Column(
                      ...
                      children: [
                        ...
                        _customComponents.cHeaderText("Captain is on it's way"),
                        Padding(padding: EdgeInsets.all(15.0)),
                        Row(
                          children: [
                            Text("Captain ID: "),
                            Text(_ridesAvailable[_nearestRideIndex].captainID, style: TextStyle(color: Theme.of(context).accentColor),),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            Text("Captain's Name: "),
                            Text(_ridesAvailable[_nearestRideIndex].captainName, style: TextStyle(color: Theme.of(context).accentColor),),
                          ],
                        ),
                        Padding(padding: EdgeInsets.all(5.0)),
                        Row(
                          children: [
                            Text("Vehicle ID: "),
                            Text(_ridesAvailable[_nearestRideIndex].vehicleID, style: TextStyle(color: Theme.of(context).accentColor),),
                          ],
                        ),
                        Padding(padding: EdgeInsets.all(5.0)),
                        Row(
                          children: [
                            ...
                            Text(_ridesAvailable[_nearestRideIndex].otherInfo, style: TextStyle(color: Theme.of(context).accentColor),),
                          ],
                        ),
                        Padding(padding: EdgeInsets.all(5.0)),
                        ElevatedButton.icon(onPressed: _callCaptain, label: Text("Call Captain?", style: TextStyle(color: Colors.white),), icon: Icon(Icons.call, color: Colors.white,),)
                      ],
                    ),
                      Align(
                        alignment: FractionalOffset.bottomCenter,
                        child: _customComponents.cHeaderText("Sit back and relax, you'll be informed as soon as the captain arrives.\n\nHave a safe and happy journey with Careen :)"),
                      ),],
                  ),
                      MediaQuery.of(context).size.height/2, MediaQuery.of(context).size.width/1.2, EdgeInsets.zero));
            });;
            _captainArrival();
      });
      ...
      final snackBar = SnackBar(content: Text('Sorry! No rides available right now.'));
  ...
  _callCaptain(){
    setState(() {
      _makePhoneCall('tel:' + _ridesAvailable[_nearestRideIndex].captainMobileNo);
    });
  }
  ...
  Future<void> _makePhoneCall(String url) async {
    if (await canLaunch(url)) {
      ...
      final snackBar = SnackBar(content: Text('Unable to call at this moment.'));
      ScaffoldMessenger.of(context).showSnackBar(snackBar);
    }
  }
  ...
  Future<void> _assignRide(String _nearestRideID, String _passengerId, LatLng _pickupLocation) async{
  ...
  _captainArrival() async {
    var getRefQuery = DBRef.child("Assigned_Rides").child(_passengerId).child("Captain_Arrived");
    await getRefQuery.onValue.listen((event){
      print(event.snapshot.value);
      if(event.snapshot.value=="true"){
        setState(() {
          FlutterBeep.playSysSound(AndroidSoundIDs.TONE_CDMA_CALL_SIGNAL_ISDN_INTERGROUP);
          Navigator.pop(context);
          showDialog(
              context: context,
              barrierDismissible: false,
                    ...
                    child: _customComponents.cCard(Stack(
                      children: [Column(
                        ...
                        children: [
                          _customComponents.cHeaderText("Ride Confirmed"),
                          Padding(padding: EdgeInsets.all(10.0)),
                          Text("Captain Arrived!", style: TextStyle(color: Colors.black, fontSize: 16.0, fontWeight: FontWeight.bold),),
                          Padding(padding: EdgeInsets.all(15.0)),
                          Row(
                            children: [
                              Text("Captain ID: "),
                        ...
                        Align(
                          alignment: FractionalOffset.bottomCenter,
                          child: _customComponents.cHeaderText("Enjoy the ride! Have a safe journey :)"),
                        ),],
                    ),
                        MediaQuery.of(context).size.height/2.3, MediaQuery.of(context).size.width/1.5, EdgeInsets.zero));
              });;
        });
  }});}
